...
const GROWTH_PATTERN_LABELS: Record<string,{title:string;description:string}> = {
  exercise_social:{title:'같이 뛰다 보니 가까워진 날',description:'몸을 움직이는 동안 동하는 혼자가 아니라는 감각을 배웠습니다.'},
  routine_focus:{title:'작은 루틴이 집중을 만든 날',description:'작은 반복이 동하의 마음을 차분하게 정리해 주었습니다.'},
  creative_social:{title:'장난이 작품이 된 순간',description:'함께 웃고 만들며 창의성이 관계가 되는 순간이었습니다.'},
  observe_curiosity:{title:'관찰하다가 궁금해진 순간',description:'자세히 바라본 만큼 질문이 깊어졌습니다.'},
  study_rest:{title:'공부하다 쉬는 법을 배운 날',description:'몰입과 회복의 균형을 스스로 찾아가기 시작했습니다.'},
  curiosity_study:{title:'궁금해서 공부하게 된 날',description:'정답보다 질문이 먼저였고, 배움은 그 뒤를 따라왔습니다.'},
  music_emotion:{title:'노래에 마음이 움직인 날',description:'음악이 동하의 감정을 부드럽게 풀어주었습니다.'},
  rest_emotion:{title:'쉬다가 마음이 풀린 날',description:'멈추는 시간 속에서 감정이 안정되었습니다.'},
  emotion_bond:{title:'마음이 통해 가까워진 날',description:'진심이 닿으면서 관계의 온도가 올라갔습니다.'}
};
...
const [weeklyCards,setWeeklyCards]=useState(()=>pickWeeklyCards(state.selectedGrowthDirection??undefined)); const [selectedWeeklyActivity,setSelectedWeeklyActivity]=useState<string|null>(null); const [pendingEvent,setPendingEvent]=useState<any|null>(null); const [reportText,setReportText]=useState('');
...
const handleOpenMilestone=()=>{
  ...
  if(!nextMilestone){ setReportText('아직 기록된 특별한 순간이 없습니다. 하지만 동하는 매주 조금씩 변하고 있습니다. 관계, 감정, 호기심이 깊어질수록 특별한 장면이 열립니다.'); return; }
...
const handleFastForwardMonth=()=>{
  ...
  const report={week:sim.currentWeek,title:'한 달 성장 요약',summary:'이번 달의 동하는 혼자 뛰는 아이에서, 함께 뛰며 웃는 아이로 조금 자랐습니다.',topStats:[],stressDelta:sim.stress-state.stress,discoveredComboIds:sim.discoveredCombos.slice(0,3),eventTitles:sim.eventHistory.slice(0,4),milestoneTitles:milestones,recommendation:'다음 달도 같은 리듬으로 진행해보세요.'};
...
<section className='summary-card'><h3>이번 달 돌봄 방향을 정해볼까요?</h3><div className='schedule-list'>{unlockedDirections.map((d)=><button key={d.id} className={state.selectedGrowthDirection===d.id?'schedule-item selected':'schedule-item'} onClick={()=>{const next={...state,selectedGrowthDirection:d.id};setState(next);saveGame(next);const nextCards=pickWeeklyCards(d.id);setWeeklyCards(nextCards);setSelectedWeeklyActivity(nextCards[0]?.id??null);}}><strong>{d.title}</strong><em>{d.description}</em></button>)}</div>{lockedDirectionCount>0?<p>동하가 조금 더 자라면 새로운 돌봄 방향이 열립니다.</p>:null}<p>선택 방향: {GROWTH_DIRECTIONS.find((d)=>d.id===state.selectedGrowthDirection)?.title ?? '미선택'}</p></section><section className='summary-card'><h3>이번 주 함께할 일</h3><div className='schedule-list'>{weeklyCards.map((c)=>{const isSelected=selectedWeeklyActivity===c.id; return <div key={c.id} className='weekly-activity-wrap'><button type='button' aria-pressed={isSelected} className={isSelected?'schedule-item weekly-activity selected':'schedule-item weekly-activity'} onClick={()=>{setSelectedWeeklyActivity(c.id);setMessage('');}} onTouchEnd={()=>{setSelectedWeeklyActivity(c.id);setMessage('');}}><strong>{c.title} {c.id===recommendedCardId?<span>추천</span>:null} {isSelected?<span className='selected-badge'>선택됨 ✓</span>:null}</strong><p>{c.description}</p><p>{c.id===recommendedCardId?(state.stress>=70?'지금은 스트레스를 낮추기 좋아요.':'이번 달 성장 방향과 잘 맞아요.'):'카드를 눌러 이번 주 활동으로 선택해 주세요.'}</p></button>{isSelected?<article className='weekly-detail-panel'><h4>{c.title}</h4><p>이번 주 이벤트: {state.stress>=70?'동하가 한숨 돌리며 마음을 가다듬는 시간을 보냅니다.':'동하가 작은 성취를 경험하며 자신만의 리듬을 만듭니다.'}</p><p>능력치 변화: {Object.entries(c.statEffects).map(([k,v])=>`${k} ${Number(v)>=0?'+':''}${v}`).join(' · ')} / 스트레스 {c.stressEffect>=0?'+':''}{c.stressEffect}</p><p className='weekly-emotion-comment'>"{c.stressEffect<0?'고마워요, 오늘은 마음이 편안해졌어요.':'오늘도 새로운 걸 해봐서 두근거렸어요!'}"</p></article>:null}</div>;})}</div></section><section className='stats-grid'>{statOrder.map((key)=><article key={key} className='stat-card'><div className='stat-head'><span>{key}</span><strong>{state.stats[key]}</strong></div><div className='bar'><i style={{width:`${state.stats[key]}%`}}/></div></article>)}</section>
<section className='schedule-card'><h3>이번 주 함께할 일 실행</h3><div className='schedule-list'>{(Object.keys(schedules) as ScheduleId[]).map((id)=><button key={id} disabled={state.actionsLeft===0} className={selected===id?'schedule-item selected':'schedule-item'} onClick={()=>setSelected(id)}><strong>{schedules[id].name}</strong><em>피로 {schedules[id].fatigue>=0?'+':''}{schedules[id].fatigue} / 스트레스 {schedules[id].stress>=0?'+':''}{schedules[id].stress}</em></button>)}</div><div className='action-buttons'><button className='run-button' onClick={runSelectedWeek} disabled={state.actionsLeft===0||!selected}>행동 실행</button><button type='button' className='run-button finish' onClick={finishWeek}>이번 주 마무리</button>{state.ageInMonths>1 || state.monthlyReports.length>0 ? <button aria-label='한달맡겨보기' className='run-button' onClick={handleFastForwardMonth}>한 달 맡겨보기</button> : <small>이제 기본 흐름을 익혔어요. 다음부터는 한 달을 빠르게 진행할 수도 있습니다.</small>}<button aria-label='특별한성장순간' className='run-button' onClick={handleOpenMilestone} >특별한성장순간</button><button className='run-button preview-button' onClick={()=>setShowPreview((v)=>!v)}>성장 방향 미리보기</button></div></section>
...
{reportText?<section className='summary-card'><h3>지난 4주 성장 리포트</h3><p>{reportText}</p></section>:null}{state.milestoneHistory.length>0?<section className='summary-card'><h3>성장 기록</h3><p>{state.milestoneHistory.slice(0,3).join(' · ')}</p></section>:null}{state.discoveredCombos.length>0?<section className='summary-card'><h3>발견한 성장 장면</h3>{state.discoveredCombos.slice(-4).reverse().map((id)=>{const meta=GROWTH_PATTERN_LABELS[id]??{title:'작은 성장의 순간',description:id}; return <p key={id}><strong>"{meta.title}"</strong><br/>{meta.description}</p>;})}</section>:null}{import.meta.env.DEV?<aside className='dev-image-panel'><strong>이미지 체크</strong><ul>{devImageTargets.map((src)=><li key={src}><span>{src.split('/').pop()}</span><em>{devImageStatus[src]??'...'}</em></li>)}</ul></aside>:null}
</main>;
}
