...
<section className='summary-card'><h3>이번 달 동하는 어떤 방향으로 자라볼까요?</h3><div className='schedule-list'>{GROWTH_DIRECTIONS.map((d)=><button key={d.id} className={state.selectedGrowthDirection===d.id?'schedule-item selected':'schedule-item'} onClick={()=>{const next={...state,selectedGrowthDirection:d.id};setState(next);saveGame(next);setWeeklyCards(pickWeeklyCards(d.id));}}><strong>{d.title}</strong><em>{d.description}</em></button>)}</div><p>선택 방향: {GROWTH_DIRECTIONS.find((d)=>d.id===state.selectedGrowthDirection)?.title ?? '미선택'}</p></section><section className='summary-card'><h3>이번 주 동하에게 찾아온 기회</h3><div className='schedule-list'>{weeklyCards.map((c)=> <article key={c.id} className='schedule-item'><strong>{c.title}</strong><p>{c.description}</p></article>)}</div></section><section className='stats-grid'>{statOrder.map((key)=><article key={key} className='stat-card'><div className='stat-head'><span>{key}</span><strong>{state.stats[key]}</strong></div><div className='bar'><i style={{width:`${state.stats[key]}%`}}/></div></article>)}</section>
<section className='schedule-card'><h3>이번 주 행동 선택</h3><div className='schedule-list'>{(Object.keys(schedules) as ScheduleId[]).map((id)=><button key={id} disabled={state.actionsLeft===0} className={selected===id?'schedule-item selected':'schedule-item'} onClick={()=>setSelected(id)}><strong>{schedules[id].name}</strong><em>피로 {schedules[id].fatigue>=0?'+':''}{schedules[id].fatigue} / 스트레스 {schedules[id].stress>=0?'+':''}{schedules[id].stress}</em></button>)}</div><div className='action-buttons'><button className='run-button' onClick={runSelectedWeek} disabled={state.actionsLeft===0||!selected}>행동 실행</button><button className='run-button finish' onClick={finishWeek}>1주 진행</button><button className='run-button' onClick={()=>{let sim=state;for(let i=0;i<4;i++){const auto=pickWeeklyCards(sim.selectedGrowthDirection??undefined)[0];const a=applyGrowthCard(auto,sim);sim=advanceWeek(runWeek(a.next,selected??'walk').state);} setState(sim); saveGame(sim); setReportText('지난 4주 동안 동하는 반복보다 리듬을 선택하며 차분히 성장했습니다.');}}>4주 빠른 진행</button><button className='run-button' onClick={()=>{let sim=state;while(sim.milestoneHistory.length===state.milestoneHistory.length){const auto=pickWeeklyCards(sim.selectedGrowthDirection??undefined)[0];const a=applyGrowthCard(auto,sim);sim=advanceWeek(runWeek(a.next,selected??'walk').state); if(sim.currentWeek-state.currentWeek>16) break;} setState(sim);saveGame(sim);}}>다음 성장 이벤트까지</button><button className='run-button preview-button' onClick={()=>setShowPreview((v)=>!v)}>성장 방향 미리보기</button></div></section>
